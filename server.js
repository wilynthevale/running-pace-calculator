...
// More precise VDOT estimation using race time lookup table
...
// API Endpoint for VDOT Calculation
app.post("/calculate-vdot", (req, res) => {
    const { raceTime, raceDistance } = req.body;
    const vdot = estimateVDOT(raceTime, raceDistance);
    res.json({ vdot });
});

app.post("/calculate-pace", (req, res) => {
    const { vdot } = req.body;
    const paces = calculatePace(vdot);
    res.json({ paces });
});
